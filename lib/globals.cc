...
uint64_t GetTigonLocalAccess() { return lib::thread_local_meta.pt_stat.owned_partition_access_cnt; }
uint64_t GetTigonLocalAccessHit() { return lib::thread_local_meta.pt_stat.owned_partition_hit_cnt; }
uint64_t GetTigonLocalAccessMiss() { return lib::thread_local_meta.pt_stat.owned_partition_miss_cnt; }

uint64_t GetTigonRemoteAccessMiss() { return lib::thread_local_meta.pt_stat.remote_partition_miss_cnt; }

uint64_t GetTigonRemoteAccessHit() { return lib::thread_local_meta.pt_stat.remote_partition_hit_cnt; }

uint64_t GetTigonRemoteAccess() { return lib::thread_local_meta.pt_stat.remote_partition_access_cnt; }
...
ssize_t Preload(uint8_t* buf, uint64_t count, off_t key) { return lib::RackOBJKV::Preload(buf, count, key); }
...
std::string GetConfigStr(std::string c_name) {
    ...
    LOG(FATAL) << "GetConfigStr: " << c_name << " not available";
    return "";
}
...
size_t GetConfigSize_t(std::string c_name) {
    ...
    } else if (c_name == "slot_size") {
        return SLOT_SIZE;
    }

    LOG(FATAL) << "GetConfigSize_t: " << c_name << " not available";
    return 0;
}
...
void ClearMovementCounter() { lib::page_cache.ClearMovementCounter(); }
...
void RegisterNRThread() {
    rackobj::lib::thread_local_meta.nr_meta_per_thread_ = rackobj::lib::page_cache.RegisterNRThread();
}
...
void RegisterLocalMem() {
    rackobj::lib::thread_local_meta.local_cache_ptr_ = rackobj::lib::page_cache.RegisterLocalMem();
}