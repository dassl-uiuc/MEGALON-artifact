...
EXPORT_LINE="export RACKOBJ_RESULT_DIR=${RESULT_DIR}"
...
cd third-party/nr_rust
./install_deps.sh
cd $PROJECT_ROOT

cd scripts
./setup_logical_node.sh 0 3 24

if ldconfig -p | grep -q 'libstdc++.*12'; then
    echo "⚠️ Warning: libstdc++ from GCC 12 is installed! This can interfere with LLVM stdc++"
    echo "📍 Locations:"
