...
# Old churn fallback (kept)
RE_CHURN = re.compile(
    r"partition_to_shared:\s*(\d+).*?shared_to_partition_:\s*(\d+)"
)

# NEW churn format:
# seqlock alloc ratio: ... reclaim count: X, allocate count: Y
...
def parse_single_file(filepath, collect_churn=False):
    ...
    for line in lines:
        ...
        if collect_churn:
            ...
            if m_new:
                current["churn_s2p"] += int(m_new.group("reclaim"))   # reclaim
                current["churn_p2s"] += int(m_new.group("alloc"))     # allocate
                continue
