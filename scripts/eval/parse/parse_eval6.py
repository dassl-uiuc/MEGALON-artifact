...
RE_NUM_OBJS = re.compile(r"warming up with\s+(\d+)\s+ops", re.IGNORECASE)
RE_TPUT = re.compile(
    r"GLOBAL THROUGHPUT:\s+avg=(\d+)\s+ops/s,\s+stddev=(\d+)\s+ops/s"
)
# Updated filename regex to include seqlock_bit
RE_FILENAME = re.compile(
    r"output_zipfian_(?P<w_ratio>[0-9.]+)_(?P<zipf>[0-9.]+)_(?P<scr>\d+)MB-(?P<seqlock>\d+)\.log"
)
...
# NEW churn format: reclaim count / allocate count
RE_CHURN_NEW = re.compile(
    r"reclaim count:\s*(?P<reclaim>\d+).*?allocate count:\s*(?P<alloc>\d+)"
)
...
def parse_single_file(filepath, collect_churn=False):
    ...
    seqlock_bit = int(m.group("seqlock"))
    ...
    for line in lines:
        ...
        if collect_churn:
            # NEW churn logic
            m_new = RE_CHURN_NEW.search(line)
            if m_new:
                current["churn_s2p"] += int(m_new.group("reclaim"))
                current["churn_p2s"] += int(m_new.group("alloc"))
                continue
            # Old fallback
            m_old = RE_CHURN.search(line)
            if m_old:
                current["churn_p2s"] += int(m_old.group(1))
                current["churn_s2p"] += int(m_old.group(2))
                continue
    ...
    for r in runs:
        if r["threads"] is None or r["tput"] is None:
            print(f"Warning incomplete result: {r}")
            continue
        results.append({
            "scr_size": scr_size,
            "write_ratio": w_ratio,
            "zipf": zipf,
            "seqlock_bit": seqlock_bit,
            "threads": r["threads"],
            "num_objs": r["num_objs"],
            "tput": r["tput"],
            "stddev": r["stddev"],
...
# ---------------------------------------------
# Group results based on NOT-group-by
# ---------------------------------------------
def group_results(results, churn):
    # Always group top-level on (write_ratio, zipf)
    ...
    for key, entries in top_groups.items():
        ...
        for e in entries:
            key2 = (e["scr_size"], e["threads"], e["num_objs"])
            sub[key2].append(e)
        for (scr, thr, objs), group in sorted(sub.items()):
            print(f"\n-- SCR_SIZE={scr}MB, THREADS={thr}, NUM_OBJS={objs} --")
            for e in sorted(group, key=lambda x: x["seqlock_bit"]):
                print(f"{e['seqlock_bit']},{fmt(e)}")
...
if __name__ == "__main__":
    ...
    if not results:
        ...
        group_results(results, args.churn)
