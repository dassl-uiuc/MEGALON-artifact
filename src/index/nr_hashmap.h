...
typedef struct GCDEntry_t {
    ssize_t wmeta_idx_;
    CNStatus_t cn_array_[NUM_NUMA];  // make sure NUM_NUMA == NUM_EXEC + 1 (declared in rust)
} GCDEntry;

extern "C" {
NrRapper* create_node_replicated(uint64_t cap, uint64_t num_replica);
NrRapper* create_node_replicated_numa(uint64_t cap, uint64_t num_replica, const uint8_t* base_addr);
