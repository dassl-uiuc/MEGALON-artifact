...
size_t EvictManager<Policy>::DoEvictShared(size_t to_evict
    ...
    do {
        ...
        if (old_wmeta_idx_optional.value().has_value()) {
            ...
            c3po_->Scr_meta()->RecycleWmeta(static_cast<size_t>(old_wmeta_idx_optional.value().value()), PLACEHOLDER_0);
        }
...
#if 0
    PolicyNode *evicted_head, *evicted_tail;
#endif
    size_t nr_evicted = 0;
    bool succeed;
...
#if 0
    evicted_head = evicted_tail = nullptr;
#endif
