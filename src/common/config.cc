...
RackOBJConfig::RackOBJConfig() {
    ...
    std::stringstream ss;
    if (!config["slots"]) {
        LOG(FATAL) << "Server config must specify \"slots\"";
    }
    ...
    if (config["partition_ratio"]) {
        partition_ratio_ = config["partition_ratio"].as<double>();
    } else {
        partition_ratio_ = 1.0;
        LOG(WARNING) << "partition_ratio not specified, default to 1.0";
    }
