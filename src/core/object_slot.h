...
public:
    ...
    uint8_t *GetDataSlot(size_t cn_idx) const { return page_data_.GetDataSlot(cn_idx); }
...
public:
    ...
    explicit LocalMemoryObject(size_t num_pages, const std::shared_ptr<AllocatableLocalMemoryRegion> &local_region)
        ...
        LOG(INFO) << "LocalMemoryObject &policy_=" << (void *)&policy_ << ", pages_=" << (void *)pages_;
    }
...
private:
    uint8_t *pages_;
    // std::unique_ptr<LocalMetadata> lmeta_;
