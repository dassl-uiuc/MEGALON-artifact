...
typedef struct seqcount {
    ...
    std::atomic<uint8_t> active_node;
#else
    volatile uint32_t sequence;
    volatile uint8_t active_node;
#endif
} seqcount_t;
...
uint8_t get_active_node(struct seqcount *s);
void set_active_node(struct seqcount *s, uint8_t node);
...
constexpr uint32_t OWNER_MASK = ~(3u << 28);  // bit 28 and 29
...
uint8_t get_active_node(seqlock_t *s);
void set_active_node(seqlock_t *s, uint8_t node);
