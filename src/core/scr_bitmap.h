...
public:
    ...
    std::optional<size_t> ReserveCacheNode() {
        ...
        do {
            ...
            uint32_t rand_start = static_cast<uint32_t>(static_cast<size_t>(std::rand()) % bitmap_len);
            if (!bitmap_find_setbit(free_bitmap_start_addr_, bitmap_len, index, offset, rand_start))
                return std::nullopt;
            succeed = try_clearbit_exclusive(free_bitmap_start_addr_, index, offset);
        } while (!succeed);
