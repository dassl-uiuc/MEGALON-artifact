...
public:
    explicit CacheNode() : block_id_(), length_(0), evict_cnt_(1) {}
...
static inline void print_entry(common::GCDEntry* entry) {
    ...
    for (size_t i = 0; i < LOGICAL_NODE_NUM + 1; ++i) {
        ...
        oss << "("
            << (entry->cn_array_[i].cn_idx_.has_value() ? std::to_string(entry->cn_array_[i].cn_idx_.value()) : "null")
            << ", " << entry->cn_array_[i].invalidate_ << ")";
    }
    oss << "}";
