...
use core::sync::atomic::{AtomicBool, AtomicUsize, Ordering, AtomicIsize};
...
// my custom simple rwlock
pub struct SimpleRwLock<T> {
    state: AtomicIsize,       // -1 = writer, 0 = unlocked, >0 = reader count
    data: UnsafeCell<T>,
}

// Safety: data access is protected by locking protocol
unsafe impl<T: Send + Sync> Sync for SimpleRwLock<T> {}
unsafe impl<T: Send + Sync> Send for SimpleRwLock<T> {}
...
impl<T> SimpleRwLock<T> {
    ...
    pub fn read(&self) -> SimpleReadGuard<'_, T> {
        loop {
        ...
        SimpleReadGuard { lock: self }
    }

    pub fn write(&self) -> SimpleWriteGuard<'_, T> {
        loop {
            if self
                .state
...
pub struct SimpleReadGuard<'a, T> {
    lock: &'a SimpleRwLock<T>,
}

impl<'a, T> Deref for SimpleReadGuard<'a, T> {
    type Target = T;
...
impl<'a, T> Drop for SimpleReadGuard<'a, T> {
    fn drop(&mut self) {
        self.lock.read_unlock();
    }
}

pub struct SimpleWriteGuard<'a, T> {
    lock: &'a SimpleRwLock<T>,
}

impl<'a, T> Deref for SimpleWriteGuard<'a, T> {
    type Target = T;
...
impl<'a, T> DerefMut for SimpleWriteGuard<'a, T> {
    fn deref_mut(&mut self) -> &mut T {
        unsafe { &mut *self.lock.data.get() }
    }
}

impl<'a, T> Drop for SimpleWriteGuard<'a, T> {
    fn drop(&mut self) {
        self.lock.write_unlock();
    }
}
