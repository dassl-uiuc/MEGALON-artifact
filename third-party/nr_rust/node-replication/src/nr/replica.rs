...
#[cfg(loom)]
use loom::sync::atomic::{AtomicUsize, AtomicIsize, Ordering};
...
#[cfg(any(feature = "scr", feature = "no-coherence"))]
use super::log::flush_cache_line;
#[cfg(feature = "no-coherence")]
use super::log::{load_with_flush, fetch_max_with_flush, load_with_flush_bool};
use super::rwlock::{RwLock, ReadGuard, WriteGuard};
use super::{Dispatch, IdxGetter};
...
#[cfg(any(feature = "scr", feature = "no-coherence"))]
fn flush_mem_region(ptr: *const u8, size: usize) {
    ...
    while p < end {
        unsafe {
            flush_cache_line(p as *const u8);
        }
        p += CACHE_LINE_SIZE;
    }
...
where
    ...
    <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter,
{
...
pub struct LockedHashSet<T> 
where T: Sync {
    // spinlock: Spinlock,
    data: RwLock<HashSet<T, DefaultHasher>>,
}

impl<T> LockedHashSet<T>
where
    T: core::hash::Hash + Eq + Sync,
{
    pub fn new() -> Self {
        Self {
            // spinlock: Spinlock::new(),
            data: RwLock::new(HashSet::with_hasher(DefaultHasher::default())),
        }
    }

    pub fn read(&self, tid: usize) -> ReadGuard<HashSet<T, DefaultHasher>> {
        self.data.read(tid)
    }

    pub fn write(&self, n: usize) -> WriteGuard<HashSet<T, DefaultHasher>> {
        self.data.write(n)
    }
}
    ...
    <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter,
{
    ...
    change_notif: Vec<CachePadded<AtomicIsize>>,
    ...
    /// a set for tracking responses (log updates) to cache slots
    /// the purpose is for detecting non-coherent data
    // response_set: RefCell<HashSet<<D as Dispatch>::Response, DefaultHasher>>,
    response_set: LockedHashSet<<D as Dispatch>::Response>,
    // set_lock: Spinlock,
    ...
    // base address for flushing
    base_addr: *const u8,
    // the granularity of flushing
    slot_size: usize,
}
...
unsafe impl<D> Sync for Replica<D>
where D: Sized + Sync + Dispatch, <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter {}
    ...
    <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter,
{
    ...
    pub fn new_with_data(log_tkn: LogToken, data: D) -> Replica<D> {
        Replica::with_data(log_tkn, data)
    }

    pub fn new_with_data_base_addr(log_tkn: LogToken, data: D, addr: *const u8, slot_size: usize) -> Replica<D> {
        Replica::with_data_base_addr(log_tkn, data, addr, slot_size)
    }
}
    ...
    <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter,
{
    ...
    pub fn with_data(log_tkn: LogToken, d: D) -> Replica<D> {
        ...
        let mut change_notif = Vec::with_capacity(MAX_THREADS_PER_REPLICA);
        // Add `MAX_THREADS_PER_REPLICA` contexts
        for _ in 0..MAX_THREADS_PER_REPLICA {
            contexts.push(Default::default());
            change_notif.push(CachePadded::new(AtomicIsize::new(-1))); // init to -1
        }
        ...
        Replica {
            ...
            // response_set: RefCell::new(HashSet::with_hasher(DefaultHasher::default())),
            response_set: LockedHashSet::new(),
            // set_lock: Spinlock::new(),
    ...
    pub fn with_data_base_addr(log_tkn: LogToken, d: D, addr: *const u8, s_size: usize) -> Replica<D> {
        let mut contexts = Vec::with_capacity(MAX_THREADS_PER_REPLICA);
        let mut change_notif = Vec::with_capacity(MAX_THREADS_PER_REPLICA);
        // Add `MAX_THREADS_PER_REPLICA` contexts
    ...
    // pub fn register_addr(&mut self, addr: *mut u8) {
    //     self.base_addr = addr;
    // }
    ...
    ) -> Result<<D as Dispatch>::Response, (ReplicaError<D>, <D as Dispatch>::ReadOperation<'rop>)>
    {
        ...
        let response = self.data.read(idx.tid() - 1).dispatch(op);
        self.change_notif[idx.tid() - 1].store(response.cn_idx(), Ordering::Relaxed);
        return Ok(response);
    }

    pub fn execute_loose<'rop>(
        &self,
        op: <D as Dispatch>::ReadOperation<'rop>,
        idx: ReplicaToken,
    ) -> Result<<D as Dispatch>::Response, (ReplicaError<D>, <D as Dispatch>::ReadOperation<'rop>)>
    {
        // do not perform any read
        Ok(self.data.read(idx.tid() - 1).dispatch(op))
    }

    pub fn sync_for_read<'rop>(
        &self,
        slog: &Log<<D as Dispatch>::WriteOperation>,
    ) {
        let ctail = slog.get_ctail();
        while !slog.is_replica_synced_for_reads(&self.log_tkn, ctail) {
            if let Err(_e) = self.try_combine(slog) {
                return;
            }
            spin_loop();
        }
    ...
    pub fn execute_locked_anchor<'rop, 'lock>(
        &'lock self,
        slog: &Log<<D as Dispatch>::WriteOperation>,
        op: <D as Dispatch>::ReadOperation<'rop>,
        idx: ReplicaToken,
    ...
    pub fn check_coherence(
        & self,
        slog: &Log<<D as Dispatch>::WriteOperation>,
        resp: <D as Dispatch>::Response,
        idx: ReplicaToken
    ) -> bool {
        // let ctail = slog.get_ctail();
        // while !slog.is_replica_synced_for_reads(&self.log_tkn, ctail) {
        //     if let Err(_) = self.try_combine(slog) {
        //         return false;
        //     }
        //     spin_loop();
        // }

        let responses = self.response_set.read(idx.tid() - 1);
        !responses.contains(&resp)
    }

    pub fn reset_coherence(
        & self,
        resp: <D as Dispatch>::Response
    ) {
        let mut responses = self.response_set.write(self.next.load(Ordering::Relaxed));
        responses.remove(&resp);
    }

    pub fn check_coherence_reset(
        & self,
        slog: &Log<<D as Dispatch>::WriteOperation>,
        resp: <D as Dispatch>::Response
    ) -> bool {
    ...
    pub fn check_notification_reset(
        &self,
        idx: ReplicaToken,
    ) -> bool
    {
        let current = self.change_notif[idx.tid() - 1].load(Ordering::Acquire);
        self.change_notif[idx.tid() - 1].store(-1, Ordering::Relaxed);
        return current != -1;
    }

    // pub fn set_base_addr(&mut self, addr: *mut u8) {
    //     self.base_addr = addr;
    // }
    ...
    fn exec(&self, slog: &Log<<D as Dispatch>::WriteOperation>) { 
        ...
        let next = self.next.load(Ordering::Relaxed);
        {
            ...
            // let mut responses = self.response_set.write(next);
            let mut f = |o: <D as Dispatch>::WriteOperation, mine: bool| {
                let _resp = data.dispatch_mut(o);
                // responses.insert(_resp.clone());
                // for entry in self.change_notif.iter().take(next) {
                //     if entry.load(Ordering::Relaxed) == _resp.cn_idx() {
    ...
    pub(crate) fn combine<'r>(
        ...
        let res = {
            ...
            // let mut responses = self.response_set.write(num_registered_threads);
            let f = |o: <D as Dispatch>::WriteOperation, mine: bool| {
                ...
                let idx = resp.cn_idx();
                for entry in self.change_notif.iter().take(num_registered_threads) {
                    if entry.load(Ordering::Relaxed) == idx && idx != -1{
                ...
                if mine {
                    ...
                    // TODO: add cache flush at all similar places
                    // responses.insert(resp.clone());

                    // do cache flushing
                    #[cfg(any(feature = "scr", feature = "no-coherence"))]
                    {
                        if idx != -1 {
                            let region_ptr = unsafe { self.base_addr.add(idx as usize * self.slot_size) };
                            flush_mem_region(region_ptr, self.slot_size);
                        }
                    }
                }
            };
        ...
        // Execute outstanding operations on the shared log against this replica
        {
            ...
            let mut f = |o: <D as Dispatch>::WriteOperation, mine: bool| {
                ...
                if mine {
                    ...
                    // responses.insert(resp.clone());
                    // for entry in self.change_notif.iter().take(num_registered_threads) {
                    //     if entry.load(Ordering::Relaxed) == resp.cn_idx() {
                    //         entry.store(-1, Ordering::Relaxed);
                    //     }
                    // }
