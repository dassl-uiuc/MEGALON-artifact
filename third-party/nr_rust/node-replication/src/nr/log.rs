...
use core::arch::x86_64::{_mm_clflush, _mm_sfence, _mm_lfence, _mm_mfence};

use core::arch::asm;

// #[cfg(any(feature = "scr", feature = "no-coherence"))]
// #[inline(always)]
// unsafe pub fn flush_cache_line(ptr: *const u8) {
//     asm!("clflushopt [{}]", in(reg) ptr, options(nostack, preserves_flags));
// }

// #[cfg(feature = "no-coherence")]
// #[inline(always)]
// pub fn load_with_flush(input: &CachePadded<AtomicUsize>) -> usize {
//     unsafe {
//         core::arch::x86_64::_mm_sfence();
//         flush_cache_line(&input as *const _ as *const u8);
...
// #[cfg(feature = "no-coherence")]
// #[inline(always)]
// pub fn store_with_flush(input: &CachePadded<AtomicUsize>, update: usize) {
//     input.store(update, Ordering::Relaxed);
...
// #[cfg(feature = "no-coherence")]
// #[inline(always)]
// pub fn fetch_max_with_flush(input: &CachePadded<AtomicUsize>, update: usize) {
//     unsafe {
//         core::arch::x86_64::_mm_sfence();
//         flush_cache_line(&input as *const _ as *const u8);
//         core::arch::x86_64::_mm_mfence();
...
//     input.fetch_max(update, Ordering::Relaxed);
...
// #[cfg(feature = "no-coherence")]
// #[inline(always)]
// pub fn load_with_flush_bool(input: &AtomicBool) -> bool {
//     unsafe {
//         core::arch::x86_64::_mm_sfence();
//         flush_cache_line(&input as *const _ as *const u8);
//         core::arch::x86_64::_mm_mfence();
...
// #[cfg(feature = "scr")]
// pub type Entry<T> = crate::log::Entry<T, ()>;

// #[cfg(feature = "scr")]
// #[inline(always)]
// unsafe fn force_nt_store_entry<T>(
//     e: *mut Entry<T>, 
//     new_op: Option<T>, 
//     new_replica: usize, 
//     new_alivef: bool
...
//     T: Sized + Clone,
// {
//     let ptr = e as *mut Entry<T> as *mut __m128i;

//     let mut new_entry: Entry<T> {}

//     // Write first 16 bytes
//     _mm_stream_si128(ptr.add(0), zeros);

//     // Write next 16 bytes (for BYTES > 16)
//     #[cfg(feature = "bytes32")]
//     _mm_stream_si128(ptr.add(1), zeros);

//     // Write next 16 bytes (for BYTES > 32)
//     #[cfg(feature = "bytes48")]
//     _mm_stream_si128(ptr.add(2), zeros);

//     // Write next 16 bytes (for BYTES > 48)
//     #[cfg(feature = "bytes64")]
//     _mm_stream_si128(ptr.add(3), zeros);

//     _mm_sfence();
// }

#[cfg(any(feature = "scr", feature = "no-coherence"))]
pub use crate::log::flush_cache_line;
#[cfg(feature = "no-coherence")]
pub use crate::log::{load_with_flush, store_with_flush, fetch_max_with_flush, load_with_flush_bool};
    ...
    T: Sized + Clone,
{
    ...
    pub fn append<F: FnMut(T, bool)>(
        ...
        loop {
            ...
            // distinguish coherence level
            #[cfg(feature = "no-coherence")]
            let tail = load_with_flush(&self.tail);
            #[cfg(feature = "no-coherence")]
            let head = load_with_flush(&self.head);

            #[cfg(not(feature = "no-coherence"))]
            ...
            #[cfg(feature = "no-coherence")]
            unsafe {
                flush_cache_line(self.tail.as_ptr() as *const u8);
            }
            ...
            for (i, op) in ops.iter().enumerate().take(nops) {
                ...
                #[cfg(feature = "no-coherence")]
                if unsafe { load_with_flush_bool(&(*e).alivef) == m } {
                    m = !m;
                }
    ...
    pub(crate) fn exec<F: FnMut(T, bool)>(&self, idx: &LogToken, d: &mut F) {
        ...
        #[cfg(feature = "no-coherence")]
        let ltail = load_with_flush(&self.ltails[idx.0 - 1]);
        ...
        #[cfg(feature = "no-coherence")]
        let gtail = load_with_flush(&self.tail);
        ...
        #[cfg(feature = "no-coherence")]
        let h = load_with_flush(&self.head);
        ...
        #[cfg(feature = "no-coherence")]
        {
            fetch_max_with_flush(&self.ctail, gtail);
            store_with_flush(&self.ltails[idx.0 - 1], gtail);
        }
    }
    ...
    fn advance_head<F: FnMut(T, bool)>(&self, rid: &LogToken, mut s: &mut F) -> Result<(), usize> {
        ...
        loop {
            ...
            #[cfg(feature = "no-coherence")]
            let global_head = load_with_flush(&self.head);
            #[cfg(feature = "no-coherence")]
            let f = load_with_flush(&self.tail);
            ...
            #[cfg(feature = "no-coherence")]
            store_with_flush(&self.head, min_local_tail);
