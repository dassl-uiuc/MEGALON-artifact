...
pub struct NodeReplicated<D> 
where
    D: Dispatch + Sync,
    <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter,
{
    log: Box<Log<D::WriteOperation>>,
    ...
    <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter,
{
    ...
    pub fn new_with_data(
        ...
        Self::with_log_size_and_struct(num_replicas, chg_mem_affinity, log::DEFAULT_LOG_BYTES, data)
    }

    pub fn new_with_data_numa(
        num_replicas: NonZeroUsize,
        chg_mem_affinity: impl Fn(AffinityChange) -> usize + Send + Sync + 'static,
        data: D,
    ) -> Result<Self, NodeReplicatedError> {
        Self::with_log_size_and_struct_numa(num_replicas, chg_mem_affinity, log::DEFAULT_LOG_BYTES, data)
    }

    pub fn new_with_data_and_base_addr_numa(
        num_replicas: NonZeroUsize,
        chg_mem_affinity: impl Fn(AffinityChange) -> usize + Send + Sync + 'static,
        data: D,
        addr: *const u8,
        ...
        Self::with_log_size_and_struct_and_base_addr_numa(num_replicas, chg_mem_affinity, log::DEFAULT_LOG_BYTES, data, addr, slot_size)
    }
    ...
    pub fn new_numa(
        ...
        Self::with_log_size_numa(num_replicas, chg_mem_affinity, log::DEFAULT_LOG_BYTES)
    }
    ...
    pub fn with_log_size(
        ...
        let log = Box::try_new(Log::new_with_bytes(log_size, ()))?;
    ...
    pub fn with_log_size_and_struct(
        num_replicas: NonZeroUsize,
        chg_mem_affinity: impl Fn(AffinityChange) -> usize + Send + Sync + 'static,
        log_size: usize,
        data: D
        ...
        for replica_id in 0..num_replicas.get() {
            ...
            let r = {
                let _aff_tkn = affinity_mngr.switch(replica_id);
                Box::try_new(Replica::new_with_data(log_token, data.clone()))?
                // aff_tkn is dropped here
            };
    ...
    // also sets the obj base addr for flushing
    pub fn with_log_size_and_struct_and_base_addr_numa(
        num_replicas: NonZeroUsize,
        chg_mem_affinity: impl Fn(AffinityChange) -> usize + Send + Sync + 'static,
        log_size: usize,
        data: D,
        ...
        let affinity_mngr = AffinityManager::new(Box::try_new(chg_mem_affinity)?);
        let log = {
            // allocate log on numa node 0
            let _aff_tkn = affinity_mngr.switch(usize::MAX);
            Box::try_new(Log::new_with_bytes(log_size, ()))?
        };
        ...
        for replica_id in 0..num_replicas.get() {
            ...
            let r = {
                let _aff_tkn = affinity_mngr.switch(replica_id);
                Box::try_new(Replica::new_with_data_base_addr(log_token, data.clone(), addr, slot_size))?
                // aff_tkn is dropped here
            };
    ...
    pub fn with_log_size_and_struct_numa(
        num_replicas: NonZeroUsize,
        chg_mem_affinity: impl Fn(AffinityChange) -> usize + Send + Sync + 'static,
        log_size: usize,
        data: D
        ...
        for replica_id in 0..num_replicas.get() {
            ...
            let r = {
                // always allocate on numa node 1
                let _aff_tkn = affinity_mngr.switch(replica_id);
                Box::try_new(Replica::new_with_data(log_token, data.clone()))?
                // aff_tkn is dropped here
            };
    ...
    <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter,
{
    ...
    // pub fn set_base_addr(&mut self, base_addr: *mut u8) {
    //     for replica in &mut self.replicas {
    //         replica.set_base_addr(base_addr);
    //     }
    // }
    ...
    pub fn check_coherence(
        ...
        self.replicas[tkn.rid].check_coherence(&self.log, resp, tkn.rtkn)
    }
    ...
    pub fn reset_coherence(
        ...
        self.replicas[tkn.rid].reset_coherence(resp)
    }
    ...
    pub fn check_coherence_reset(
        ...
        self.replicas[tkn.rid].check_coherence_reset(&self.log, resp)
    }
    ...
    pub fn check_notification_reset(
        ...
        self.replicas[tkn.rid].check_notification_reset(tkn.rtkn)
    }
    ...
    pub fn execute_mut(
        ...
        enum ResolveOp<'a, D>
        where
            D: core::marker::Sync + Dispatch + Sized,
            <D as Dispatch>::Response: PartialEq + Eq + Hash + Sync + IdxGetter,
        {
            ...
            match q.pop().unwrap_or(ResolveOp::Exec(None)) {
                ResolveOp::Exec(cl) => match self.try_execute_mut(op.clone(), tkn, cl) {
                    ...
                    Err(ReplicaError::GcFailed(stuck_ridx)) => {
                        ...
                        //writeln!(lock, "rust_NR::execute_mut failed").unwrap();
    ...
    {
        if let Some(combiner_lock) = cl {
            self.replicas[tkn.rid].execute_locked_anchor(&self.log, op, tkn.rtkn, combiner_lock)
        } else {
            self.replicas[tkn.rid].execute_anchor(&self.log, op, tkn.rtkn)
        }
    }

    fn try_execute_loose<'a, 'rop>(
        &'a self,
        op: <D as Dispatch>::ReadOperation<'rop>,
        tkn: ThreadToken,
        cl: Option<CombinerLock<'a, D>>,
    ) -> Result<<D as Dispatch>::Response, (ReplicaError<D>, <D as Dispatch>::ReadOperation<'rop>)>
    {
        if let Some(combiner_lock) = cl {
            ...
            self.replicas[tkn.rid].execute_loose(op, tkn.rtkn)
        }
    }
    ...
    pub fn execute_anchor(
        ...
        loop {
            match q.pop().unwrap() {
                ResolveOp::Exec(cl, op) => match self.try_execute_anchor(op, tkn, cl) {
                    Ok(resp) => {
                        assert!(q.is_empty());
                        return resp;
                    }
    ...
    pub fn execute_loose(
        ...
        loop {
            match q.pop().unwrap() {
                ResolveOp::Exec(cl, op) => match self.try_execute_loose(op, tkn, cl) {
                    Ok(resp) => {
                        assert!(q.is_empty());
                        return resp;
                    }
    ...
    pub fn log_sync_numa(&self) {
        for rid in 0..self.replicas.len() {
            self.replicas[rid].sync_for_read(&self.log);
        }
    }
}
