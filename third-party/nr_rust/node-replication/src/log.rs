...
use core::arch::x86_64::{_mm_clflush, _mm_sfence, _mm_lfence};
...
///
/// Currently set to 32 MiB which seems like a good space/performance trade-off for most
/// operation types. In general after more than 100k entries, there is little impact for
/// performance from the actual log size.
pub const DEFAULT_LOG_BYTES: usize = 64 * 1024 * 1024;
    ...
    M: Default,
{
    ...
    pub fn register(&self) -> Option<LogToken> {
        ...
        loop {
            ...
            #[cfg(feature = "no-coherence")]
            let n = load_with_flush(&self.next);
            ...
            #[cfg(feature = "no-coherence")]
            unsafe {
                flush_cache_line(self.next.as_ptr() as *const u8);
            }
    ...
    pub(crate) fn find_min_tail(&self) -> (usize, usize) {
        ...
        #[cfg(feature = "no-coherence")]
        let r = load_with_flush(&self.next);

        #[cfg(not(feature = "no-coherence"))]
        let (mut min_replica_idx, mut min_local_tail) = (0, self.ltails[0].load(Ordering::Relaxed));
        #[cfg(feature = "no-coherence")]
        let (mut min_replica_idx, mut min_local_tail) = (0, load_with_flush(&self.ltails[0]));
        ...
        for idx in 1..r {
            ...
            #[cfg(feature = "no-coherence")]
            let cur_local_tail = load_with_flush(&self.ltails[idx - 1]);
    ...
    pub(crate) fn is_replica_synced_for_reads(&self, idx: &LogToken, ctail: usize) -> bool {
        ...
        #[cfg(feature = "no-coherence")]
        {
            load_with_flush(&self.ltails[idx.0 - 1]) >= ctail
        }
    }
