...
fn main() {
    ...
    for replica_cntr in 1..=2 {
        ...
        for thread_num in 1..=16 {
            ...
            print!(
                " ({} ns/op)",
                elapsed.as_nanos() / (thread_num as u128 * PER_THREAD_OPS as u128)
            );

            print!(
                " ({} op/s)", (thread_num as f64 * PER_THREAD_OPS as f64) / elapsed.as_secs_f64()
            );

            println!(
                " ({} MB/s)", (thread_num as f64 * PER_THREAD_OPS as f64) / elapsed.as_secs_f64() * 64./8./1024./1024.
            );
        }
    }
}
