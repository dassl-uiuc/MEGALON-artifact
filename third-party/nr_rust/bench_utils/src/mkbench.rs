...
fn chg_affinity_numa(af: AffinityChange) -> usize {
    match af {
        AffinityChange::Replica(rid) => {
            let mut cpu: usize = 0;
            let mut node: usize = 0;
...
pub trait DsInterface {
    type D: Dispatch + Default + Sync + Clone;
    ...
    fn new_numa(replicas: NonZeroUsize, logs: NonZeroUsize, log_size: usize) -> Arc<Self>;
...
impl<'a, T: Dispatch + Sync + Default + Clone> DsInterface for NodeReplicated<T> {
    ...
    fn new_numa(replicas: NonZeroUsize, _logs: NonZeroUsize, log_size: usize) -> Arc<Self> {
        println!("numa init");
        Arc::new(
            NodeReplicated::with_log_size_numa(replicas, chg_affinity_numa, log_size)
                .expect("Can't allocate NodeReplicated"),
        )
    }
...
impl fmt::Display for ReplicaStrategy {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            ReplicaStrategy::NUMA => write!(f, "NUMA"),
        }
    }
}
...
impl fmt::Debug for ReplicaStrategy {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            ReplicaStrategy::NUMA => write!(f, "RS=NUMA"),
        }
    }
}
    ...
    R::D: Sync + Dispatch + Default + Send,
{
    ...
    f: BenchFn<R>,
    /// Thread handles
    handles: Vec<JoinHandle<(Core, usize, Vec<usize>)>>,
    ...
    R: DsInterface + Sync + Send,
{
    ...
    fn terminate(self) -> std::io::Result<()> {
        ...
        for (tid, handle) in self.handles.into_iter().enumerate() {
            let (cid, ttl_ops, latency) = handle.join().unwrap();
            everything.push(ttl_ops);
            lat.extend(latency);
            // all_results.push((cid, tid, thread_results));
        }

        if cfg!(not(feature = "smokebench")) {
            let avg_tput = (everything.iter().sum::<usize>() as f64)/20 as f64;
            let lat_avg = crate::benchmark::mean(&lat).unwrap();
            println!(
                "Run({:?} {:?} {:?} {:?} BS={} {:.5} op/s) => {:.5} ns/op",
                self.rs,
                self.tm,
                self.ts,
                self.ls,
            ...
            // let eve_str = everything.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", ");
            // println!("{} {}", eve_str, everything.len());
        } else {
            println!(
                "Run({:?} {:?} {:?} {:?} BS={}) => not measured",
    ...
    fn terminate_new(self) -> std::io::Result<()> {
        let mut everything =
            Vec::<usize>::with_capacity(self.handles.len() * self.duration.as_secs() as usize);
        let mut lat =
            Vec::<usize>::with_capacity(self.handles.len() * self.duration.as_secs() as usize);
        ...
        if cfg!(not(feature = "smokebench")) {
            let avg_tput = everything.iter().sum::<usize>();
            let lat_avg = crate::benchmark::mean(&lat).unwrap();
            println!(
                "Run({:?} {:?} {:?} {:?} BS={} {:.5} op/s) => {:.5} ns/op",
                self.rs,
    ...
    fn startup(&mut self) {
        ...
        let ds = if self.numa { R::new_numa(replicas, NonZeroUsize::new(1).unwrap(), self.log_size) }
                           else { R::new(replicas, NonZeroUsize::new(1).unwrap(), self.log_size) };
        ...
        for (rid, cores) in self.rm.clone().into_iter() {
            for core_id in cores {
                ...
                let operations = if stripe_len > 0 {
                    let vec_ref = &self.operations;
                    let start = core_id as usize * stripe_len;
                    let end = (core_id as usize + 1) * stripe_len;
                    let vec_slice = &vec_ref[start..end];
                    let new_vec: Vec<_> = vec_slice.to_vec();
                    Arc::new(new_vec)
                } else {
                    self.operations.clone()
                };

                let load_ops = if stripe_len > 0 {
                    self.load_vec.clone()
                } else {
                    Arc::new(Vec::new())
                };
                ...
                self.handles.push(thread::spawn(move || {
                    ...
                    if core_id == 10 {
                        ...
                        println!("load done from core 10");
                    }
                    ...
                    let mut ttl_ops: usize = 0;
                    let mut operations_per_second: Vec<usize> = Vec::with_capacity(32);
                    let mut latency: Vec<usize> = Vec::with_capacity(32);
                    let mut operations_completed: usize = 0;
                    let mut iter: usize = 0;
                    ...
                    let end_experiment = start + duration;
                    let mut next_log = start + log_period;
                    let one_second_in_nanoseconds: usize = 1_000_000_000;
                    while Instant::now() < end_experiment {
                        ...
                        if Instant::now() >= next_log {
                            ...
                            let average_latency_ns = if operations_completed > 0 {
                                one_second_in_nanoseconds / operations_completed
                            } else {
                                0
                            };

                            latency.push(average_latency_ns);

                            operations_per_second.push(operations_completed);
                            ttl_ops += operations_completed;
                            // reset operations completed
                            operations_completed = 0;
                            next_log += log_period;
                        }
                    ...
                    start_sync.wait();
                    (core_id, ttl_ops, latency)
                }));
            }
        }
    }

    fn startup_new(&mut self) {
        let thread_num = self.threads();
        ...
        for (rid, cores) in self.rm.clone().into_iter() {
            for core_id in cores {
                ...
                self.handles.push(thread::spawn(move || {
                    ...
                    let mut ttl_ops: usize = 0;
                    let mut latency: Vec<usize> = Vec::with_capacity(operations.len());
                    ...
                    for op in operations.iter() {
                        ...
                        ttl_ops += 1;
                        let elapsed_time = iter_start.elapsed(); 
                        latency.push(elapsed_time.as_nanos() as usize);
                    }
                    let elapsed_ttl = start.elapsed().as_secs();
                    let tput = ttl_ops as u64 / elapsed_ttl;
    ...
    <R::D as Dispatch>::Response: Send,
{
    ...
    pub fn loadop_vec(&mut self, ops_vec: Vec<
            Operation<
                <R::D as Dispatch>::ReadOperation<'static>,
                <R::D as Dispatch>::WriteOperation,
            >,
        ...
        self.load_vec = ops_vec;
        self
    }

    pub fn op_stripe_len(&mut self, stripe_len: usize) -> &mut Self {
        self.stripe_len = stripe_len;
        self
    } 

    pub fn set_numa(&mut self, numa: bool) -> &mut Self {
        self.numa = numa;
        self
    } 
    ...
    pub fn configure_new(&self, c: &mut TestHarness, name: &str, f: BenchFn<R>)
    where
        <R::D as Dispatch>::WriteOperation: Sync + Send + Copy + 'static,
        <R::D as Dispatch>::ReadOperation<'static>: Sync + Send + Copy + Clone,
        <R::D as Dispatch>::Response: Send,
