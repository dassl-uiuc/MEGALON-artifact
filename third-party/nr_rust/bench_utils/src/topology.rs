...
impl fmt::Display for ThreadMapping {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            ThreadMapping::NUMA => write!(f, "NUMA"),
        }
    }
}
...
impl fmt::Debug for ThreadMapping {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            ThreadMapping::NUMA => write!(f, "TM=NUMA"),
        }
    }
}
    ...
    pub fn allocate(&self, strategy: ThreadMapping, how_many: usize, use_ht: bool) -> Vec<CpuInfo> {
        ...
        match strategy {
            ...
            ThreadMapping::Interleave => {
                ...
                ht1.dedup_by(|a, b| a.socket != b.socket);
                ...
                // ht1.extend(ht2);
                let ht = interleave(ht1, ht2).collect::<Vec<_>>();
                ...
                //cpus.dedup_by(|a, b| a.core == b.core);
                let c = ht.iter().take(how_many).map(|c| *c).collect();
                c
            }
