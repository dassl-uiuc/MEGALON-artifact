...
int main(int argc, char** argv) {
    ...
    for (size_t thread_idx = 0; thread_idx < warm_threads.size(); ++thread_idx) {
        warm_threads[thread_idx] =
            std::jthread([thread_idx, operation_count, &warmup_sequence, &warmup_barrier, workload_type] {
                ...
                if (workload_type == YCSBWorkload::A || workload_type == YCSBWorkload::B ||
                    workload_type == YCSBWorkload::F) {
