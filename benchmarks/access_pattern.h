...
public:
    ...
    /** Generate operation type (default: map GenerateRW to READ/UPDATE) */
    virtual Operation GenerateOperation() { return GenerateRW() == 0 ? Operation::READ : Operation::UPDATE; }

    off_t GetPageCount() const { return page_count_; }

    off_t GetPageOffset() const { return PAGE_OFFSET; }
...
public:
    PartialPartitionedAccessPattern(size_t page_count, int total_nodes, int logical_node_index,
                                    double partition_percentage, double partition_access_prob, double theta = 0.99,
                                    float write_ratio = 0.0)
        : AccessPattern(page_count),
          ...
          rd_(),
          generator_(rd_()) {
        size_t partitioned_keys = static_cast<size_t>(page_count * partition_percentage_);
        partition_size_ = partitioned_keys / total_nodes_;
        shared_total_size_ = page_count - partitioned_keys;
        shared_slice_size_ = shared_total_size_ / total_nodes_;

        // Compute this node's private partition start
        block_size_ = partition_size_ + shared_slice_size_;
        partition_start_ = logical_node_index_ * block_size_;

        // Create partition zipfian
        partition_zipfian_ = std::make_unique<ScrambledZipfianGenerator>(partition_size_, theta, write_ratio);

        if (shared_total_size_ > 0)
            shared_zipfian_ = std::make_unique<ScrambledZipfianGenerator>(shared_total_size_, theta, write_ratio);
        else
    ...
    off_t GenerateNextOffset() override {
        ...
        if (access_partition || partition_percentage_ == 1.0) {
            uint64_t idx = partition_zipfian_->Next();
            return (partition_start_ + idx) * PAGE_OFFSET;
        }

        // Shared access
        uint64_t x = shared_zipfian_->Next();  // x ∈ [0, N * shared_slice_size)

        size_t slice = x / shared_slice_size_;  // which shared slice (0..N-1)
        size_t offset_in_slice = x % shared_slice_size_;

        size_t slice_global_start = slice * block_size_ + partition_size_;

        return (slice_global_start + offset_in_slice) * PAGE_OFFSET;
    }
...
private:
    ...
    size_t shared_slice_size_;  // size of S_i
    size_t shared_total_size_;  // N * S
    size_t block_size_;         // P + S
