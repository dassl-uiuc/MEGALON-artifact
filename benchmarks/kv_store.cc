...
int main(int argc, char** argv) {
    ...
    double partition_percentage = std::stod(argv[6]);
    ...
    if (pattern == "hotcachepartial") {
        dir += "hcmeta/" + pattern + "-" + write_ratio_str + std::to_string(zipfian_theta);
    } else if (pattern == "zipfian" || pattern == "partial-partitioned") {
        ...
        dir += "hcmeta/" + pattern + "-" + sr_str + "-" + write_ratio_str;
    } else {
        dir += "hcmeta/" + pattern + "-" + write_ratio_str;
    }
    struct stat st = {0};
        ...
        for (size_t thread_idx = 0; thread_idx < threads.size(); ++thread_idx) {
            threads[thread_idx] = std::jthread([thread_idx, operation_count, warmup_sequence, &warmup_barrier] {
                ...
                for (size_t i = local_thread_idx; i < operation_count; i += threads_per_numa) {
                    rc = rackobj::Preload(reinterpret_cast<uint8_t*>(buf), ACCESS_SIZE, warmup_sequence[i]);
                    PCHECK(rc != -1);
                }
    ...
    for (size_t i = 0; i < thread_cnt; ++i) {
        workers[i] = std::jthread([&, i] {
            ...
            int rc_getcpu = getcpu(nullptr, &numa_node_local);
            CHECK(rc_getcpu != -1) << "getcpu() failed";
            CHECK(numa_node_local == rackobj::GetConfigSize_t("logical_nid") + 1)
